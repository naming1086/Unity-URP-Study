...
half3 ExponentialHeightFog(half3 col, half3 posWorld)
{
    ...
    half inscatterFactor = pow(saturate(dot(-normalize(viewDir), myLight.direction)), _FogInscatteringExp);
    inscatterFactor *= 1-saturate(exp2(falloff));
    inscatterFactor *= distanceFactor;
    half3 finalFogColor = lerp(_FogColor, myLight.color, saturate(inscatterFactor));
